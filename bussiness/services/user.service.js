...
const userService = {
  async signin(username, password) {
    ...
    if (!resultSigninValidator.isSuccess) return resultSigninValidator;
    // Find username in DB
    ...
    if (!resultTokenGenerator.isSuccess) return resultTokenGenerator;
    return {
      isSuccess: true,
  ...
  async signup(fullname, username, password) {
    ...
    if (!resultSignUpValidator.isSuccess) return resultSignUpValidator;
  ...
  async updateUser(fullname, password, id) {
    ...
    if (!resultUpdateValidator.isSuccess) return resultUpdateValidator;
    // get user by token
    const user = await userRepository.getUserById(id);
    if (user === operatorType.FAIL.READ) {
      return {
